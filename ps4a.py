...
def getWordScore(word, n):
    ...
    partialSum = 0
    for char in word:
        partialSum += SCRABBLE_LETTER_VALUES[char]
    if len(word) == n:
         totalSum = partialSum * len(word) + 50
    else:
         totalSum = partialSum * len(word)
    return totalSum
...
def isValidWord(word, hand, wordList):
    ...
    if word in wordList:
        for char in word:
            if char in handCopy and handCopy[char] > 0:
                handCopy[char] -= 1
            else:
                return False
    else:
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(hand) != 0:
        ...
        displayHand(hand)
        w = input('Enter word, or a "." to indicate that you are finished: ')
        ...
        if w == '.':
            ...
            if isValidWord(w, hand, wordList) == False:
                ...
                print('"'+ w + '"', "earned", wordScore,"points.", "Total:", totalScore, "points.")
